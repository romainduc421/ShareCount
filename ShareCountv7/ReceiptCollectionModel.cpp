...
QVariant ReceiptCollectionModel::data(const QModelIndex &index, int role) const
{
    if (role == Qt::DisplayRole) {
        if (index.parent() == QModelIndex() ) { //this is a ReceiptCollection
            ...
            Recepisse r = m_pGroup->GetReceiptCollection( index.internalId()-1 ).GetReceipt( index.row() );
...
void ReceiptCollectionModel::AddReceipt(const Recepisse& R, QModelIndex const& selected)
{
...
Recepisse& ReceiptCollectionModel::GetReceipt(const QModelIndex& index)
{
...
void ReceiptCollectionModel::ChangeReceipt(const QModelIndex& index, Recepisse& r)
{
    if (index == QModelIndex() || index.parent() == QModelIndex()) { //this is a collection, not a receipt
